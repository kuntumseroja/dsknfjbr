...
echo
echo "Creating Diskominfo Asset Ledger Network (DALN) Channel $CHANNEL_NAME"
echo
...
createChannelTxn() {
	...
	set -x
	configtxgen -profile DiskominfoassetLedgerChannel -outputCreateChannelTx ./channel-artifacts/${CHANNEL_NAME}.tx -channelID $CHANNEL_NAME
	res=$?
	set +x
...
createAncorPeerTxn() {
  ...
  displayMsg "Generating anchor peer update transaction for ${orgmsp}"
	set -x
	configtxgen -profile DiskominfoassetLedgerChannel -outputAnchorPeersUpdate ./channel-artifacts/${orgmsp}anchors.tx -channelID $CHANNEL_NAME -asOrg ${orgmsp}
	res=$?
	set +x
...
echo
echo "========= Diskominfo Asset Ledger Network (DALN) Channel $CHANNEL_NAME successfully joined =========== "
