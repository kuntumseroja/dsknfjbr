...
function printHelp() {
  ...
  echo "      - 'deploySmartContract' - deploy the diskominfoassetledger chaincode on the channel"
  echo "      - 'down' - clear the network with docker-compose down"
  echo "      - 'restart' - restart the network"
  echo
  echo "    Flags:"
  echo "    -ca <use CAs> -  create Certificate Authorities to generate the crypto material"
  echo "    -c <channel name> - channel name to use (defaults to \"dalnchannel\")"
  ...
  echo
  echo " Examples:"
  echo "  net-daln.sh up createChannel"
  echo "  net-daln.sh down"
  echo "  net-daln.sh deploySmartContract"
}
...
function createConsortium() {
  ...
  set -x
  configtxgen -profile DiskominfoassetLedgerOrdererGenesis -channelID system-channel -outputBlock ./system-genesis-block/genesis.block
  res=$?
  set +x
  if [ $res -ne 0 ]; then
...
function networkDown() {
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    rm -rf channel-artifacts log.txt diskominfoassetledgernetwork.tar.gz
...
CLI_DELAY=3
# channel name defaults to "dalnchannel"
CHANNEL_NAME="dalnchannel"
# use this as the default docker-compose yaml definition
COMPOSE_FILE_BASE=docker/docker-compose-daln-net.yaml
...
if [ "$MODE" == "up" ]; then
  ...
  echo "Starting docker log monitoring on network '${COMPOSE_PROJECT_NAME}_daln'"
  echo
elif [ "$MODE" == "monitor-down" ]; then
  echo "Stopping docker log monitoring on network '${COMPOSE_PROJECT_NAME}_daln'"
  echo
else
  printHelp
  exit 1
